...
const { WIN_MULTIPLIER } = require('../config/constants');
...
function setupRoomHandlers(io) {
  io.on('connection', (socket) => {
    socket.on('join-room', async (data) => {
      ...
      const db = getDb();
      const room = await db.prepare('SELECT * FROM rooms WHERE code = ?').get(String(roomCode).trim().toUpperCase());
      ...
      if (room.status === 'racing' && !room.race_state) {
        ...
        await db.prepare('UPDATE rooms SET race_state = ?, winner_suit = ?, status = ?, updated_at = ? WHERE id = ?')
          .run(raceState, result.winner, 'finished', now, room.id);

        const roomPlayers = await db.prepare('SELECT user_id, suit, points_bet FROM room_players WHERE room_id = ?').all(room.id);
        for (const rp of roomPlayers) {
          if (rp.suit === result.winner) {
            const winAmount = rp.points_bet * WIN_MULTIPLIER;
            const u = await db.prepare('SELECT points FROM users WHERE id = ?').get(rp.user_id);
            const newPoints = Math.max(0, Number(u.points) + winAmount);
            await db.prepare('UPDATE users SET points = ?, updated_at = ? WHERE id = ?').run(newPoints, now, rp.user_id);
